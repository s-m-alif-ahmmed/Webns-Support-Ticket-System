                    ...
                    <div class="main-content-body main-content-body-chat h-100">
                        ...
                        @if($ticket->status == 'Open')
                            <div class="main-chat-footer py-3">
                                @php
                                    $userHasTicketAssignment = $ticket_assigns->contains('assign_user_id', Auth::user()->id);
                                @endphp
                                @if($userHasTicketAssignment)
                                    @if($permissionData && isset($permissionData['tickets_all']['messages_all']['message_create']) && $permissionData['tickets_all']['messages_all']['message_create'] == 'message_create')
                                        <div class="message-form-container">
                                            ...
                                            <form class="d-flex border-0 p-0 m-0" id="messageForm" action="{{ route('ticket-messages.store') }}" method="POST" enctype="multipart/form-data">
                                                @csrf
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        if (data.is_company_user) {
                            ...
                            newMessage.classList.add('flex-row-reverse', 'chat-right');
                        }
