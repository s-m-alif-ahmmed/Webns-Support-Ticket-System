        ...
        <div class="col-lg-12 col-md-12">
            <div class="card">
                <div class="card-header py-3 bg-transparent border-bottom justify-content-between">
                    ...
                    @if($companyPermissionData && isset($companyPermissionData['company_users_tickets']['assign_user_add']) && $companyPermissionData['company_users_tickets']['assign_user_add'] == 'assign_user_add')
                                ...
                                <div class="table-responsive product-description-each" style="overflow: hidden;">
                                    <div class="invoice-product-table">
                                        @foreach($company_ticket_assigns as $company_ticket_assign)
                                            <div class="row border-bottom pb-3 mb-3" id="viewCompanyRow{{ $company_ticket_assign->id }}" style="overflow: hidden;">
                                                ...
                                                <div class="col-md-1 col-12 my-1">
                                                    <div class="d-flex justify-content-between">
                                                        @if($companyPermissionData && isset($companyPermissionData['company_users_tickets']['assign_user_remove']) && $companyPermissionData['company_users_tickets']['assign_user_remove'] == 'assign_user_remove')
                                                            @if($ticket->status == 'Open')
                                                                <form action="{{ route('company.user.delete.assign', $company_ticket_assign->id )}}" method="post" id="deleteForm{{ $company_ticket_assign->id }}">
                                                                    @csrf
                                                                    @method('DELETE')
                                                            ...
                                                            @endif
                                                        @endif
                                                        @if($companyPermissionData && isset($companyPermissionData['company_users_tickets']['assign_user_edit']) && $companyPermissionData['company_users_tickets']['assign_user_edit'] == 'assign_user_edit')
                                                            @if($ticket->status == 'Open')
                                            ...
                                            <form id="editCompanyForm{{ $company_ticket_assign->id }}" class="d-none" action="{{ route('company.user.update.assign', $company_ticket_assign->id) }}" method="POST">
                                                @csrf
                                                @method('PUT')

                                                <input type="hidden" name="ticket_id" value="{{ $ticket->id }}" />
                                                <input type="hidden" name="create_user_id" value="{{ $company_ticket_assign->create_user_id }}" />
                                                <input type="hidden" name="company_user_id" value="{{ $company_ticket_assign->company_user_id }}" />
